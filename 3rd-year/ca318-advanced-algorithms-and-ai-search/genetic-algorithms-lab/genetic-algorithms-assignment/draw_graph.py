...
def do_ga():
    ...
    space = [DimensionRange(10, max_x - 10),
             DimensionRange(10, max_y - 10)]*(len(nodes))
    # print(space)

    steps = [8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192]
    for num_steps in steps:
        #random.seed(173)
        random.seed(0)
        sol = optimization.genetic_algorithm(space, cost, elite_fraction = 0.4, pop_size = 50, mutation_rate = 0.6, max_step = 100, max_iterations = num_steps)
        print("{} : {:6.3f}".format(num_steps, cost(sol)))
    print()
